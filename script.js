...
let opts = {
  strings: ["Feliz", "Cumpleaños!", "Numero 18!"],
  charSize: 30,
  charSpacing: 35,
  lineHeight: 40,
  ...
  if (this.phase === "firework") {
    if (!this.spawned) {
      ...
      for (let i = 1; i < this.prevPoints.length; i++) {
        ...
        ctx.lineWidth = p[2] * lineWidthProportion * i;
        ctx.beginPath();
        ctx.moveTo(p[0], p[1]);
...
function anim() {
  ...
  // detener animación una vez terminada
  if (done && !finished) {
    finished = true;
